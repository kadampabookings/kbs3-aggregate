...
CREATE TABLE public.money_transfer (
    ...
    accounting_export_date timestamp without time zone,
...
BEGIN
		...
		IF (not mt.spread) THEN
			SELECT d.event_id, d.organization_id INTO eventid, organizationid from document d where d.id=mt.document_id;
		ELSE
      ...
      -- select MoneyAccount ma
		...
		SELECT INTO ma_id ma.id FROM money_account as ma
			...
			WHERE
				...
				AND mat.customer = (mt.payment=true AND true AND mt.refund=false) -- was: AND mat.customer = (mt.payment=true AND d.expenditure=false AND mt.refund=false)
				-- always false: AND mat.supplier = (mt.payment=true AND d.expenditure=true AND mt.refund=true)
...
bookings as (
	...
	where
		...
		-- and d1.person_organization_id is not null (removed for 2022 Spring Festival)
...
BEGIN	
	...
	if (length(s) > 127) then
		s := substring(s, 1, 63) || '…' || substring(s, length(s) - 62, 63);
	end if;
...
CREATE FUNCTION public.compute_money_account_balances(money_account_id integer, from_date timestamp without time zone) RETURNS integer
    ...
    AS $$
DECLARE
    ma money_account%ROWTYPE;
	mt money_transfer%ROWTYPE;
	pt money_transfer%ROWTYPE; -- previous money transfer
	balance int;
BEGIN

    RAISE NOTICE 'Updating balances for money_account % from date %', money_account_id, from_date;

	SELECT INTO ma * FROM money_account WHERE id=money_account_id;

    RAISE NOTICE 'STEP 1';

	-- find the last money_transfer before the triggered date and start from that balance
	SELECT INTO mt * FROM money_transfer WHERE date < from_date and date > from_date - interval '1 day' AND not pending and successful AND (from_money_account_id=money_account_id OR to_money_account_id=money_account_id) ORDER BY date DESC,id DESC LIMIT 1;

	IF NOT FOUND THEN
    	RAISE NOTICE 'STEP 1b';
		SELECT INTO mt * FROM money_transfer WHERE date < from_date and date > from_date - interval '1 month' AND not pending and successful AND (from_money_account_id=money_account_id OR to_money_account_id=money_account_id) ORDER BY date DESC,id DESC LIMIT 1;
		IF NOT FOUND THEN
	    	RAISE NOTICE 'STEP 1c';
			SELECT INTO mt * FROM money_transfer WHERE date < from_date AND not pending and successful AND (from_money_account_id=money_account_id OR to_money_account_id=money_account_id) ORDER BY date DESC,id DESC LIMIT 1;
		END IF;
	END IF;
	...
	IF NOT FOUND THEN
		balance := 0;
	ELSIF mt.from_money_account_id=money_account_id THEN
	...
	FOR mt IN SELECT * FROM money_transfer WHERE date >= from_date AND (from_money_account_id=money_account_id OR to_money_account_id=money_account_id) ORDER BY date, id 
	LOOP
		RAISE NOTICE 'Updating money_transfer %', mt.id;
		IF mt.from_money_account_id=money_account_id THEN
			...
			IF ma.closed and FOUND THEN
				RAISE EXCEPTION '''%'' money account is closed', ma.name;
			END IF;
		ELSE
			IF not mt.pending and mt.successful THEN
				balance := balance + mt.amount;
...
select rn.id as new_id, prn.id as new_parent_id, arn.id as new_attendance_option_id, rs.id as old_id from row_new rn join row_src rs on rs.row_number=rn.row_number left join row_src prs on prs.id=rs.parent_id left join row_new prn on prn.row_number=prs.row_number left join row_src ars on ars.id=rn.attendance_option_id left join row_new arn on arn.row_number=ars.row_number
	...
	loop
		raise notice 'Updating new_info %', new_info; 
...
BEGIN
	...
	IF (OLD.trigger_defer_compute_balances=false AND NEW.trigger_defer_compute_balances=true) THEN
		...
		PERFORM compute_money_account_balances(NEW.id, from_date);
...
BEGIN
	...
	loop
		...
		if (position(pattern in txt) > 0) then
			replacement := null;
			EXECUTE 'select (' || br.replacement || ') from ' || case when siid is null then 'document d where d.id=$1' else 'document d, scheduled_item si where d.id=$1 and si.id=$3' end || case when br.condition is null then '' else ' and (' || br.condition || ')' end into replacement using docId, language, siid;
			if (replacement is not null) then
				txt := replace(txt, pattern, replacement);
			end if;
		end if;
...
CREATE FUNCTION public.resource_availability_by_event_items(eventid integer) RETURNS SETOF public.resource_availibility
    LANGUAGE sql IMMUTABLE STRICT
    AS $$
-- Original SQL query commented for performance issue, replaced by the next SQL query below (rewritten with help of ChatGPT) 
-- with dates as (select * from generate_event_dates(eventid) as d(date)),
...
-- Execution time with Fall Festival 2024 (eventid = 1277):
-- Original SQL: 3s
-- New SQL: 0.3s

WITH dates AS (
  SELECT * 
  FROM generate_event_dates(eventid) AS d(date)
),
attendance_info AS (
    SELECT 
        a.date, 
    ...
    LEFT JOIN resource_configuration rc ON rc.id = dl.resource_configuration_id
    JOIN document d ON d.id = dl.document_id
    where d.event_id=eventid
      and a.present
      AND NOT dl.frontend_released
      AND COALESCE(rc.online, true)
    GROUP BY a.date, dl.site_id, dl.item_id
),
date_resource_info AS (
    SELECT 
        d.date, 
        r.site_id, 
        rc.item_id, 
        rc.max AS max_online
    FROM dates d
    JOIN resource_configuration rc 
      ON overlaps(d.date, d.date, rc.start_date, rc.end_date)
    JOIN resource r 
    ...
    WHERE s.online 
      AND rc.online 
      AND (s.event_id = e.id 
        OR (s.event_id IS NULL AND s.organization_id = e.organization_id) 
        OR EXISTS (
            SELECT 1 
            FROM option o 
            WHERE o.event_id = e.id 
              AND o.site_id = s.id 
              AND o.item_id = rc.item_id
        ))
),
date_site_item_info AS (
    SELECT 
        date, 
        site_id, 
        item_id, 
        SUM(max_online) AS max_online
    FROM date_resource_info
    GROUP BY date, site_id, item_id
), 
date_site_item_info_with_current as (SELECT 
    dsii.date, 
    dsii.site_id, 
    dsii.item_id, 
    COALESCE(ai.total_quantity, 0) AS current, 
    dsii.max_online
FROM date_site_item_info dsii
LEFT JOIN attendance_info ai 
  ON ai.date = dsii.date 
  AND ai.site_id = dsii.site_id 
  AND ai.item_id = dsii.item_id)
select CAST(row_number() over(order by date) as int), site_id, item_id, date, CAST(COALESCE(current,0) as int) as current, CAST(max_online as int) as max from date_site_item_info_with_current; 
...
CREATE FUNCTION public.resource_capacity_by_event_items(eventid integer) RETURNS SETOF public.resource_capacity
	...
	--	date_resource_info_with_current as (select dri.date, dri.resource_configuration_id, dri.site_id, dri.item_id, (select coalesce(sum(dl.quantity),0) from attendance a join document_line dl on a.document_line_id=dl.id join document d on d.id=dl.document_id where d.event_id=eventid and dl.resource_configuration_id=dri.resource_configuration_id and a.present and a.date=dri.date and not dl.frontend_released) as current, dri.max from date_resource_info dri),
...
attendance_info AS (
    ...
    WHERE d.event_id = eventid
      AND a.present
      AND NOT dl.frontend_released
    GROUP BY a.date, dl.resource_configuration_id, e.organization_id
),
resource_info AS (
    SELECT 
        ...
        r.site_id, 
        rc.item_id, 
        MIN(rc.max) AS capacity,
        COALESCE(MAX(ai.total_quantity), 0) AS current
    FROM dates
    JOIN resource_configuration rc 
      ON overlaps(dates.date, dates.date, rc.start_date, rc.end_date)
    JOIN resource r 
      ON rc.resource_id = r.id
    JOIN site s 
      ON r.site_id = s.id
    LEFT JOIN attendance_info ai 
      ON ai.resource_configuration_id = rc.id 
      AND ai.date = dates.date
    WHERE s.online 
      AND rc.online 
      AND (s.event_id = eventid OR (s.event_id IS NULL AND s.organization_id = ai.organization_id))
    GROUP BY rc.id, r.site_id, rc.item_id
)
	select CAST(row_number() over(order by site_id,item_id,capacity) as int),site_id,item_id,capacity,CAST(count(*) as int) as count,CAST(sum(case current when 0 then 1 else 0 end) as int) as free from resource_info group by site_id,item_id,capacity;
$$;
   ...
   RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
	update document_line dl set trigger_defer_allocate=true from document d, item i, organization o
      ...
      and (not NEW.if_ordained or d.person_ordained)
		...
		and (NEW.if_resource_configuration_id is null or exists(select * from document_line dl2 where document_id=d.id and not cancelled and resource_configuration_id=NEW.if_resource_configuration_id))
...
BEGIN
   RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
   new.date = (select date from scheduled_item where id=new.scheduled_item_id);
   return new;
END $$;
...
BEGIN
    RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
    FOR lt IN EXECUTE 'SELECT * FROM letter l JOIN letter_type t ON l.type_id=t.id, document d WHERE d.id=$1 and (t.cart or t."order") AND (l.event_id=d.event_id OR l.event_id is null AND l.organization_id=d.organization_id) ORDER BY CASE WHEN l.event_id=d.event_id THEN 0 ELSE l.id END' USING NEW.id
        LOOP
...
DECLARE
	...
	scheduled_item_id scheduled_item.id%TYPE;
BEGIN
    RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
	...
	IF FOUND THEN
		...
		IF FOUND THEN
			...
			END IF;
			EXECUTE 'SELECT ($1).' || lang INTO body USING lt;
			body := interpret_brackets(body, NEW.id, lang, scheduled_item_id);
			subject := interpret_brackets(subject, NEW.id, lang, scheduled_item_id);
			INSERT INTO mail (account_id,letter_id, document_id, scheduled_item_id, background, subject, content, read) values (ma.id, lt.id, NEW.id, scheduled_item_id, true, subject, body, true) returning id into mail_id;
...
BEGIN
	...
	SELECT INTO lt * FROM letter l JOIN letter_type t ON l.type_id=t.id WHERE t.send_password and l.active and (l.event_id is null or eventid is not null and l.event_id=eventid) and (l.organization_id=1 or organizationid is not null and l.organization_id=organizationid) order by case when eventid is not null and l.event_id=eventid then 0 else 1 end, case when organizationid is not null and l.event_id is null and l.organization_id=organizationid then 0 else 1 end, case when l.event_id is null and l.organization_id=1 then 0 else 1 end, l.id  LIMIT 1;
	IF FOUND THEN
...
BEGIN
   RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
	SELECT INTO NEW.account_id ma.id FROM mail_account ma, document d
		where 
			d.id=NEW.document_id and ma.organization_id=d.organization_id
			or ma.organization_id=NEW.organization_id
		order by 
			case when NEW.letter_id is not null and ma.id=(select account_id from letter l where l.id=NEW.letter_id) then 0 else 1 end,
			case when ma.event_id=d.event_id then 0 else 1 end,
			"default" desc,
			ma.id
		LIMIT 1;
	RETURN NEW;
...
DECLARE
    ...
    lang magic_link.lang%TYPE;
    body letter.en%TYPE;
    subj letter.subject_en%TYPE;
    ...
    IF (NOT EXISTS(select * from recipient where mail_id=NEW.id)) THEN
        IF (NEW.out) THEN -- outgoing email => sent to the person
            ...
            IF FOUND THEN -- email sent for a specific booking
                email := doc.person_email;
                select into type lt.* from letter l join letter_type lt on lt.id=l.type_id where l.id=NEW.letter_id;
                if (type.order) then
                    select into email fa.username from person p join frontend_account fa on fa.id=p.frontend_account_id where p.id=doc.person_id;
                end if;
                INSERT INTO recipient (mail_id,person_id, email) values (NEW.id, doc.person_id, email);
            ELSE -- email not sent for a specific booking (only once case for now: the magic link)
                ...
                IF FOUND THEN
                    ...
                    /* Commented as the mail body is for now generated by KBS3 server
                    select into lt * from letter l join letter_type t on t.id=l.type_id where t.magic_link limit 1;
                    IF FOUND THEN
                        lang :=  mlink.lang;
                        EXECUTE 'SELECT ($1).subject_' || lang INTO subj USING lt;
                        IF (subj is null) THEN
                            lang := 'en';
                            subj := lt.subject_en;
                        END IF;
                        EXECUTE 'SELECT ($1).' || lang INTO body USING lt;
                        body := replace(body, '[magicLink]', '<a href="' || mlink.link || '">' || mlink.link || '</a>');
                        update mail set subject = subj, content = body where id=NEW.id;
                    END IF;
                    */
                END IF;
            END IF;
...
CREATE FUNCTION public.trigger_money_flow_transfer_ready_daily_batch() RETURNS trigger
    ...
    AS $$
DECLARE
	next_transfer_timestamp timestamp;
BEGIN
   	RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
	-- selecting the next expected batch transfer time: the one after the first not yet batch transaction
	select into next_transfer_timestamp date_trunc('day', date) + NEW.auto_transfer_time + case when date < date_trunc('day', date) + NEW.auto_transfer_time then interval '0 day' else interval '1 day' end
		from money_transfer
		where
			date > current_date - 7 -- this is for performance reason (because the table is big) and we assume there is no money transfer older than a week not yet in a batch
			and transfer_id is null
			and to_money_account_id=NEW.from_money_account_id
			and method_id=NEW.method_id
			and not receipts_transfer
			and parent_id is null
			and not pending
			and successful
		order by date
		limit 1;
	if (now() > next_transfer_timestamp) then -- if we have now reached the next batch transfer time
		-- we create a batch transfer at that time (which will include at least one transaction)
		insert into money_transfer (date, from_money_account_id, to_money_account_id, method_id, receipts_transfer, payment) values (next_transfer_timestamp, NEW.from_money_account_id, NEW.to_money_account_id, NEW.method_id, true, false);
	end if;
	NEW.trigger_transfer := false;-- rearming the trigger
	RETURN NEW;
END
$$;
...
BEGIN
    if (TG_OP = 'DELETE') then THIS := OLD; else THIS := NEW; end if;
    -- RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, THIS.id;
    if (TG_OP <> 'INSERT' OR NEW.successful=true) then -- Initial pending payment has no impact on document deposit
...
BEGIN
	...
	IF (THIS.parent_id is null and (TG_OP <> 'UPDATE' OR NEW.from_money_account_id is distinct from OLD.from_money_account_id or NEW.to_money_account_id is distinct from OLD.to_money_account_id or NEW.amount is distinct from OLD.amount or (not NEW.pending and NEW.successful) <> (not OLD.pending and OLD.successful))) THEN
...
BEGIN
    RAISE NOTICE 'Entering trigger %.%(%)', TG_RELNAME, TG_NAME, NEW.id;
    NEW.bookable_scheduled_item_id := NEW.id;
    RETURN NEW;
END;
$$;
...
CREATE TABLE public.allocation_rule (
    ...
    if_ref_min integer,
    if_ref_max integer,
    if_resource_configuration_id integer
);
...
CREATE TABLE public.book (
    ...
    order_url character varying(1024),
    free_url character varying(1024),
    ord integer
);
...
CREATE TABLE public.bookable_period (
    id integer NOT NULL,
    event_id integer,
    start_scheduled_item_id integer NOT NULL,
    end_scheduled_item_id integer NOT NULL,
...
CREATE TABLE public.event (
    ...
    accounting_exported boolean DEFAULT false NOT NULL,
    venue_id integer,
    kbs3 boolean DEFAULT false NOT NULL,
    description text,
    short_description text,
    external_link character varying(1024),
    state public.event_state,
    advertised boolean DEFAULT false NOT NULL,
    audio_expiration_date timestamp without time zone,
    vod_expiration_date timestamp without time zone,
    livestream_url character varying(1024),
    privacy_notice_label_id integer,
    audio_recordings_day_ticket boolean DEFAULT false NOT NULL,
    recurring_with_audio boolean DEFAULT false NOT NULL,
    recurring_with_video boolean DEFAULT false NOT NULL,
    repeatable boolean DEFAULT false NOT NULL,
    repeated_event_id integer,
    repeat_audio boolean DEFAULT false NOT NULL,
    repeat_video boolean DEFAULT false NOT NULL,
    vod_processing_time_minutes integer,
    audio_closing_date timestamp without time zone,
    livestream_message_label_id integer
);
...
CREATE TABLE public.channel (
    ...
    fetch_url character varying(1024) NOT NULL
);
...
CREATE TABLE public.frontend_account (
    ...
    backoffice boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.kdm_center (
    id integer NOT NULL,
    kdm_id integer NOT NULL,
    name character varying(255) NOT NULL,
    lat real,
    lng real,
    type character varying(64) NOT NULL,
    mothercenter boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.label (
    ...
    zhs character varying(4096),
    zht character varying(4096),
    el character varying(4096),
    vi character varying(4096)
);
...
CREATE TABLE public.magic_link (
    ...
    usage_run_id character varying(64),
    login_run_id character varying(64) NOT NULL,
...
CREATE TABLE public.meal_estimation (
    ...
    organization_id integer NOT NULL,
    date date NOT NULL,
    lunch_vegetarian integer,
    lunch_gluten_free integer,
    dinner_vegetarian integer,
    dinner_gluten_free integer
);
...
CREATE TABLE public.media (
    ...
    scheduled_item_id integer,
    lang character varying(8),
    multilang boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.media_consumption (
    ...
    livestreamed boolean DEFAULT false NOT NULL,
    duration_millis integer
);
...
CREATE TABLE public.news (
    ...
    channel_news_id integer NOT NULL,
    date timestamp without time zone NOT NULL,
    title character varying(1024) NOT NULL,
    excerpt character varying(1024),
    lang character varying(4096),
    image_url character varying(1024),
    link_url character varying(1024),
    contains_videos boolean DEFAULT false NOT NULL,
    topic_id integer,
    with_videos boolean DEFAULT false NOT NULL
);
...
CREATE TABLE public.option (
    ...
    mates_optional boolean DEFAULT false NOT NULL,
    propagate_fields_from_parent boolean DEFAULT true NOT NULL
);
...
CREATE TABLE public.organization (
    ...
    terms_url_de character varying(255),
    terms_url_pt character varying(255),
    kdm_center_id integer,
    latitude real,
    longitude real,
    import_issue character varying(1024),
...
CREATE TABLE public.person (
    ...
    male boolean,
    ordained boolean DEFAULT false NOT NULL,
    birthdate date,
...
CREATE TABLE public.podcast (
    ...
    duration_millis integer,
    teacher_id integer,
    wistia_video_id character varying(32)
);
...
CREATE TABLE public.resource_configuration (
    ...
    allows_male boolean DEFAULT false NOT NULL,
    allows_female boolean DEFAULT false NOT NULL,
    allows_guest boolean DEFAULT false NOT NULL,
...
CREATE TABLE public.room_cleaning (
    ...
    name character varying(64),
    status integer,
    last_passage timestamp without time zone,
    comment character varying(255),
    last_departure timestamp without time zone,
    next_arrival timestamp without time zone
);
...
CREATE TABLE public.scheduled_item (
    ...
    program_scheduled_item_id integer,
    name character varying(256),
    label_id integer,
    teacher_id integer,
    expiration_date timestamp without time zone,
    vod_delayed boolean DEFAULT false NOT NULL,
    published boolean DEFAULT false NOT NULL,
    bookable_scheduled_item_id integer,
...
CREATE TABLE public.task_metrics (
    ...
    user_software character varying(32),
    user_agent character varying(256),
    session_id character varying(256),
    server_software character varying(32),
    server_ip character varying(32),
    name character varying(256),
    language character varying(32),
    statement character varying(65536),
    parameters character varying(16384),
...
CREATE TABLE public.teacher (
    ...
    label_id integer,
    who_label_id integer,
    has_videos boolean DEFAULT false NOT NULL,
    has_podcasts boolean DEFAULT false NOT NULL
);
...
CREATE TABLE public.timeline (
    ...
    label_id integer,
    name character varying(256),
    audio_offered boolean DEFAULT false NOT NULL,
    video_offered boolean DEFAULT false NOT NULL,
    event_timeline_id integer,
    CONSTRAINT family_or_item_not_null CHECK (((item_family_id IS NOT NULL) OR (item_id IS NOT NULL)))
);
...
CREATE TABLE public.topic (
    ...
    name character varying(64) NOT NULL,
    label_id integer,
    virtuous boolean DEFAULT false NOT NULL,
    channel_topic_id character varying(255),
...
ALTER TABLE ONLY public.bookable_period
    ADD CONSTRAINT bookable_period_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.day_template
    ADD CONSTRAINT day_template_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.kdm_center
    ADD CONSTRAINT kdm_center_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.meal_estimation
    ADD CONSTRAINT meal_estimation_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.media_consumption
    ADD CONSTRAINT media_consumption_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.room_cleaning
    ADD CONSTRAINT room_cleaning_pk PRIMARY KEY (id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.scheduled_resource
    ADD CONSTRAINT scheduled_resource_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.allocation_rule
    ADD CONSTRAINT allocation_rule_if_resource_configuration_id_fkey FOREIGN KEY (if_resource_configuration_id) REFERENCES public.resource_configuration(id);
...
ALTER TABLE ONLY public.attendance
    ADD CONSTRAINT attendance_scheduled_item_id_fkey FOREIGN KEY (scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.attendance
    ADD CONSTRAINT attendance_scheduled_resource_id_fkey FOREIGN KEY (scheduled_resource_id) REFERENCES public.scheduled_resource(id);
...
ALTER TABLE ONLY public.bookable_period
    ADD CONSTRAINT bookable_period_end_scheduled_item_id_fkey FOREIGN KEY (end_scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.bookable_period
    ADD CONSTRAINT bookable_period_event_id_fkey FOREIGN KEY (event_id) REFERENCES public.event(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.bookable_period
    ADD CONSTRAINT bookable_period_start_scheduled_item_id_fkey FOREIGN KEY (start_scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.day_template
    ADD CONSTRAINT day_template_event_id_fkey FOREIGN KEY (event_id) REFERENCES public.event(id);
...
ALTER TABLE ONLY public.document_line
    ADD CONSTRAINT document_line_timeline_id_fkey FOREIGN KEY (timeline_id) REFERENCES public.timeline(id);
...
ALTER TABLE ONLY public.error
    ADD CONSTRAINT error_user_person_id_fkey FOREIGN KEY (user_person_id) REFERENCES public.person(id);
...
ALTER TABLE ONLY public.event
    ADD CONSTRAINT event_livestream_message_label_id_fkey FOREIGN KEY (livestream_message_label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.event
    ADD CONSTRAINT event_privacy_notice_label_id_fkey FOREIGN KEY (privacy_notice_label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.event
    ADD CONSTRAINT event_repeated_event_id_fkey FOREIGN KEY (repeated_event_id) REFERENCES public.event(id);
...
ALTER TABLE ONLY public.event_type
    ADD CONSTRAINT event_type_label_id_fkey FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.event_type
    ADD CONSTRAINT event_type_recurring_item_id_fkey FOREIGN KEY (recurring_item_id) REFERENCES public.item(id);
...
ALTER TABLE ONLY public.event
    ADD CONSTRAINT event_venue_id_fkey FOREIGN KEY (venue_id) REFERENCES public.site(id);
...
ALTER TABLE ONLY public.history
    ADD CONSTRAINT history_user_person_id_fkey FOREIGN KEY (user_person_id) REFERENCES public.person(id);
...
ALTER TABLE ONLY public.item
    ADD CONSTRAINT item_language_id_fkey FOREIGN KEY (language_id) REFERENCES public.language(id);
...
ALTER TABLE ONLY public.mail
    ADD CONSTRAINT mail_magic_link_id_fkey FOREIGN KEY (magic_link_id) REFERENCES public.magic_link(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY public.mail
    ADD CONSTRAINT mail_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organization(id);
...
ALTER TABLE ONLY public.mail
    ADD CONSTRAINT mail_scheduled_item_id_fkey FOREIGN KEY (scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.meal_estimation
    ADD CONSTRAINT meal_estimation_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organization(id);
...
ALTER TABLE ONLY public.media_consumption
    ADD CONSTRAINT media_consumption_attendance_id_fkey FOREIGN KEY (attendance_id) REFERENCES public.attendance(id);
...
ALTER TABLE ONLY public.media_consumption
    ADD CONSTRAINT media_consumption_media_id_fkey FOREIGN KEY (media_id) REFERENCES public.media(id);
...
ALTER TABLE ONLY public.media_consumption
    ADD CONSTRAINT media_consumption_scheduled_item_id_fkey FOREIGN KEY (scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.media
    ADD CONSTRAINT media_scheduled_item_id_fkey FOREIGN KEY (scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.money_account
    ADD CONSTRAINT money_account_label_id_fkey FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.news
    ADD CONSTRAINT news_channel_id_fkey FOREIGN KEY (channel_id) REFERENCES public.channel(id);
...
ALTER TABLE ONLY public.organization
    ADD CONSTRAINT organization_kdm_center_id_fkey FOREIGN KEY (kdm_center_id) REFERENCES public.kdm_center(id);
...
ALTER TABLE ONLY public.organization
    ADD CONSTRAINT organization_language_id_fkey FOREIGN KEY (language_id) REFERENCES public.language(id);
...
ALTER TABLE ONLY public.person
    ADD CONSTRAINT person_account_person_id_fkey FOREIGN KEY (account_person_id) REFERENCES public.person(id);
...
ALTER TABLE ONLY public.podcast
    ADD CONSTRAINT podcast_channel_id_fkey FOREIGN KEY (channel_id) REFERENCES public.channel(id);
...
ALTER TABLE ONLY public.podcast
    ADD CONSTRAINT podcast_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teacher(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_bookable_scheduled_item_id_fkey FOREIGN KEY (bookable_scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_comment_label_id_fkey FOREIGN KEY (comment_label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_event_id_fkey FOREIGN KEY (event_id) REFERENCES public.event(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_item_id_fkey FOREIGN KEY (item_id) REFERENCES public.item(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_label_id_fkey FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_parent_id_fkey FOREIGN KEY (program_scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_site_id_fkey FOREIGN KEY (site_id) REFERENCES public.site(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teacher(id);
...
ALTER TABLE ONLY public.scheduled_item
    ADD CONSTRAINT scheduled_item_timeline_id_fkey FOREIGN KEY (timeline_id) REFERENCES public.timeline(id);
...
ALTER TABLE ONLY public.scheduled_resource
    ADD CONSTRAINT scheduled_resource_configuration_id_fkey FOREIGN KEY (configuration_id) REFERENCES public.resource_configuration(id);
...
ALTER TABLE ONLY public.scheduled_resource
    ADD CONSTRAINT scheduled_resource_scheduled_item_id_fkey FOREIGN KEY (scheduled_item_id) REFERENCES public.scheduled_item(id);
...
ALTER TABLE ONLY public.site
    ADD CONSTRAINT site_country_id_fkey FOREIGN KEY (country_id) REFERENCES public.country(id);
...
ALTER TABLE ONLY public.tagged
    ADD CONSTRAINT tagged_tag_id_fkey FOREIGN KEY (tag_id) REFERENCES public.tag(id);
...
ALTER TABLE ONLY public.task_metrics
    ADD CONSTRAINT task_metrics_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES public.task_metrics(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_day_template_id_fkey FOREIGN KEY (day_template_id) REFERENCES public.day_template(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_event_id_fkey FOREIGN KEY (event_id) REFERENCES public.event(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_event_timeline_id_fkey FOREIGN KEY (event_timeline_id) REFERENCES public.timeline(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_item_family_id_fkey FOREIGN KEY (item_family_id) REFERENCES public.item_family(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_item_id_fkey FOREIGN KEY (item_id) REFERENCES public.item(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_label_id_fkey FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_label_id_fkey1 FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.timeline
    ADD CONSTRAINT timeline_site_id_fkey FOREIGN KEY (site_id) REFERENCES public.site(id);
...
ALTER TABLE ONLY public.topic
    ADD CONSTRAINT topic_label_id_fkey FOREIGN KEY (label_id) REFERENCES public.label(id);
...
ALTER TABLE ONLY public.video
    ADD CONSTRAINT video_news_id_fkey FOREIGN KEY (news_id) REFERENCES public.news(id);
...
ALTER TABLE ONLY public.video
    ADD CONSTRAINT video_playlist_id_fkey FOREIGN KEY (playlist_id) REFERENCES public.playlist(id);
...
ALTER TABLE ONLY public.video
    ADD CONSTRAINT video_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teacher(id);
